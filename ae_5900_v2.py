...
class RadioInterface:
    def __init__(self):
        ...
        self.squelch_timeout_until = 0.0  # Zeitstempel, bis wann SQL gesperrt ist
        self.asq_timeout_until = 0.0      # Zeitstempel, bis wann ASQ gesperrt ist
        self.mute_timeout_until = 0.0      # Zeitstempel, bis wann MUTE gesperrt ist
...
def api_cmd(cmd):
    ...
    with radio.lock:
        if cmd in ['VOLUP', 'VOLDOWN']:
            ...
            # 1. Bevor wir umschalten: Wenn wir NOCH im AM/FM sind, merken wir uns den echten ASQ-Zustand!
            vorheriger_mode = MODES[radio.mode_idx].upper()
            if vorheriger_mode in ["AM", "FM"]:
                radio.config["asq_memory"] = radio.config.get("asq_enabled", False)
                radio.save_config()

            # 2. Modulation umschalten
            ...
            # 3. NACH DEM UMSCHALTEN: Wo gelandet?
            neuer_mode = MODES[radio.mode_idx].upper()
            
            if neuer_mode in ["AM", "FM"]:
                # Rückkehr zu AM/FM: Wir holen den echten Zustand aus dem Gedächtnis zurück!
                radio.config["asq_enabled"] = radio.config.get("asq_memory", False)
                print(f"MODULATION-LOGIK: Zurück in {neuer_mode}. ASQ aus Gedächtnis restauriert: {radio.config['asq_enabled']}")
            else:
                # Wechsel zu SSB/CW/PA: ASQ im RAM deaktivieren fürs grüne Squelch-Prozent-Label!
                radio.config["asq_enabled"] = False
                print(f"MODULATION-LOGIK: Modus {neuer_mode} erfordert manuellen SQ. ASQ im RAM stummgeschaltet.")
            ...
            def run_super_sync_process():
                ...
                with radio.lock:
                    ...
                    if ziel_mode in ["AM", "FM"]:
                        ...
                        radio.config["asq_memory"] = True  # Auch im Gedächtnis scharf schalten!
                    else:
                        radio.config["asq_enabled"] = False
                        radio.config["asq_memory"] = True  # Im Gedächtnis True, damit es bei der Rückkehr zu FM aufptoppt!
...
#            threading.Thread(target=run_squelch_reset_fixed, daemon=True).start()
                    ...
                    if current_mode in ["AM", "FM"]:
                        ...
                        if radio.config.get("asq_enabled", False):
                            def delayed_asq_off(h_clean, dur_val):
                                ...
                                with radio.lock:
                                    radio.config["asq_enabled"] = False
                                    radio.config["asq_memory"] = False  # Gedächtnis aktualisieren
                                    radio.save_config()
                            ...
                            if radio.ser:
                                ...
                                radio.ser.write(bytes.fromhex("41000000" + hex_clean + "000006"))
                            radio.config["asq_enabled"] = True
                            radio.config["asq_memory"] = True   # Gedächtnis aktualisieren
                            radio.save_config()
        ...
        # --- CONFIG- & INTERNE ROUTEN (STAY BÜNDIG) ---
            ...
            if "VOX" in cmd: 
                ...
                radio.config["current_beep"] = val
            elif "PTTHOTKEY" in cmd: 
                radio.config["ptt_hotkey"] = val
            elif "SKIP" in cmd: 
                key_name = "skip_pa" if "PA" in cmd else "skip_cw"
                radio.config[key_name] = (val.lower() == 'true')
            elif "CLAR" in cmd:
                if "OFFSET" in cmd: 
                    radio.config["clar_offsets"][str(radio.current_ch).zfill(2)] = int(val)
                else: 
        ...
        if radio.is_tx and (time.time() - radio.ptt_start_time >= radio.config["ptt_timeout"]):
            ...
            # Stelle sicher, dass play_roger_beep() definiert ist, falls du es nutzt
