...
class RadioInterface:
    ...
    def load_config(self):
        default = {
            ...
            "asq_enabled": False,
            "clar_step": "STEP",
            "clar_offsets": {str(ch).zfill(2): 0 for ch in range(1, 41)}
        }
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, 'r', encoding='utf-8') as f: 
                    ...
                    # DAU-SCHUTZ: Altes globales 'clar_offset' abfangen und migrieren falls vorhanden
                    if "clar_offset" in disk_config and "clar_offsets" not in disk_config:
                        ...
                        last_ch_str = str(disk_config.get("last_ch", 1)).zfill(2)
                        disk_config["clar_offsets"][last_ch_str] = int(old_val)
                    
                    # Falls unvollstaendig, Matrix auffuellen
                    if "clar_offsets" not in disk_config:
                        disk_config["clar_offsets"] = default["clar_offsets"]
                        
                    self.config = {**default, **disk_config}
                    print("CONFIG: 40-Kanal-Clarifier-Matrix erfolgreich geladen.")
            except Exception as e: 
                print(f"CONFIG-FEHLER: Nutze Defaults. Fehler: {e}")
                self.config = default
        else: 
            self.config = default
...
def api_cmd(cmd):
    ...
    with radio.lock:
        ...
        if cmd in ['VOLUP', 'VOLDOWN']:
            ...
            if digit in key_codes:
                # DAU-SCHUTZ: Nur auf den Bus schreiben, wenn das USB-Kabel physisch existiert!
                if radio.ser:
                    ...
                    print(f"SIMULATION: Tastenfeld-Ziffer {digit} im RAM emuliert (Kein USB-Geraet).")
                
                # Wenn es die ERSTE Ziffer ist, starten wir den 10-Sekunden-Timer
        ...
        # --- TIMEOUT FÜR UNVOLLSTÄNDIGE KANAL-EINGABE ---
        #if len(radio.key_buffer) == 1 and (time.time() - getattr(radio, 'key_input_start_time', 0) >= 10.0):
            #radio.key_buffer = ""
            #print("KEYPAD-TIMEOUT: Keine zweite Ziffer innerhalb von 10s empfangen. Puffer geloescht.")
            ...
            if "VOX" in cmd:
                ...
                radio.config["mute_enabled"] = (val.lower() == 'true')
            elif "ASQ" in cmd:
                ...
                if "OFFSET" in cmd: 
                    # Holt den aktuellen Kanal als zweistelligen String (z.B. "03")
                    ch_str = str(radio.current_ch).zfill(2)
                    # Schreibt den Offset exakt in die Schublade dieses Kanals!
                    radio.config["clar_offsets"][ch_str] = int(val)
                else: 
    ...
    # =========================================================================
    # END LOCK SECTION (Hier schliesst mit radio.lock)
    # =========================================================================
    ...
    # 2. Kanalspezifischen Clarifier-Wert aus der Matrix extrahieren
    current_ch_str = str(radio.current_ch).zfill(2)
    
    # Sicherstellen, dass das Dictionary existiert, falls beim Upgrade was hakte
    if "clar_offsets" not in radio.config:
        radio.config["clar_offsets"] = {str(ch).zfill(2): 0 for ch in range(1, 41)}
        
    current_channel_offset = radio.config["clar_offsets"].get(current_ch_str, 0)

    # 3. PTT-Timeout Restzeit ermitteln
    rem = int(radio.config["ptt_timeout"] - (time.time() - radio.ptt_start_time)) if radio.is_tx else radio.config["ptt_timeout"]
    
    # 4. Stabilen Soll-Wert ermitteln, falls ein Makro die Hardware blockiert
    is_macro = getattr(radio, 'macro_active', False)
    vox_stat = radio.config.get("vox_enabled", False)
    mute_stat = radio.config.get("mute_enabled", False)
    ...
    return jsonify({
        ...
        # HIER ERFOLGT DIE FEHLERFREIE ZUWEISUNG AN DAS FRONTEND!
