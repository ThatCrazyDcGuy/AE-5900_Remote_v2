            ...
            for user in list(mumble.users.values()):
                ...
                while user.sound.is_sound():
                    ...
                    if sound_packet and sound_packet.pcm:
                        # Extrahiere die 48kHz 16-Bit Samples
                        fmt = f"{len(sound_packet.pcm) // 2}h"
                        samples = struct.unpack(fmt, sound_packet.pcm)
                        
                        # --- DOWNSAMPLING auf 16kHz + GAIN ---
                        # [::3] nimmt nur jedes 3. Sample -> perfekt fuer Funkqualitaet!
                        mod_samples = []
                        for s in samples[::3]:
                        ...
                        # Format fuer 16kHz neu berechnen (Anzahl Samples ist nun ein Drittel)
